...
(() => {
  ...
  // CSV→app category mapping (optional)
...
`<svg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 640 420'>
  <defs>
    <linearGradient id='g' x1='0' x2='1'><stop stop-color='${color}' stop-opacity='.35' offset='0'/><stop stop-color='#0b0f1a' offset='1'/></linearGradient>
    <filter id='glow'><feGaussianBlur stdDeviation='4' result='b'/><feMerge><feMergeNode in='b'/><feMergeNode in='SourceGraphic'/></feMerge></filter>
  </defs>
  <rect width='640' height='420' rx='26' fill='url(#g)'/>
  <g opacity='.25' stroke='${color}' fill='none' stroke-width='4'><rect x='30' y='30' width='580' height='360' rx='22'/></g>
  <g transform='translate(0,-10)' opacity='.45'><circle cx='320' cy='180' r='58' fill='${color}' opacity='.25'/><rect x='220' y='235' width='200' height='120' rx='30' fill='${color}' opacity='.18'/></g>
  <g filter='url(#glow)'><rect x='275' y='240' width='90' height='70' rx='12' fill='none' stroke='${color}' stroke-width='4'/><path d='M300 240 v-20 a20 20 0 0 1 40 0 v20' stroke='${color}' stroke-width='4' fill='none'/></g>
  <text x='50%' y='78%' text-anchor='middle' font-size='28' fill='#d9e6ff' opacity='.9' font-family='system-ui,-apple-system,Segoe UI,Roboto,Arial'>${label} · LOCKED</text>
</svg>`);
    return `data:image/svg+xml;charset=utf-8,${svg}`;
  }
  ...
  async function loadCharactersFromCSV(){
    ...
    const jsonCandidates = ["assets/characters.json","assets/characters/index.json"];
    ...
    // CSV fallbacks (space + %20 + common alternates)
    const csvCandidates = [
  ...
  function parseCharacterCSVToPool(text){
    ...
    const idx = { cat: find(["category","cat","group"]), img: find(["image","img","file","filename","path"]), name: find(["name","title","codename"]), rarity: find(["rarity","grade","tier"]) };
    ...
    for(const cols of rows){
      ...
      const chosen = normalized?.match(/^https?:/i)
        ? normalized
        : `assets/characters/${categoryLower}/${normalized || `${categoryLower}-1.png`}`;
